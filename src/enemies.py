...
class Pin:
    ...
    def draw(self, surf, scroll):
        if self.timer > self.delay:
            ...
            surf.blit(rot_surf, (self.pos.x + int(self.surf.get_width() / 2) - int(rot_surf.get_width() / 2) - scroll[0], self.pos.y + int(self.surf.get_height() / 2) - int(rot_surf.get_height() / 2) - scroll[1]))
...
class Pufferfish:
    ...
    def shoot(self, amount=6):
        for a in range(amount):
            ...
            self.app.pins.append(Pin([self.get_rect().centerx + math.cos(angle) * 8, self.get_rect().centery + math.sin(angle) * 8], self.app.assets["pin"], angle, 1, a * 60 / amount))
